...
from boostx.core.services.discord_boost.boost_discord_service import BoostDiscordResult, BoostDiscordService

_GENERIC_FAILURE_MESSAGE = "Unable to reach the server. Please try again."


class BoostDiscordController(QObject):
    started = Signal()
    failed = Signal(str)
    stopped = Signal()
    ...
    def __init__(self, session_manager: SessionManager, parent: QObject | None = None) -> None:
        ...
        self._service = BoostDiscordService()
        self._busy = False
    ...
    def _on_start_result(self, result: BoostDiscordResult) -> None:
        self._busy = False
        if result.success:
            self.started.emit()
        else:
